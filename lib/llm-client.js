...
async function routeToProvider(provider, prompt, options) {
  ...
  // Skip Ollama if it fails twice (local network issues common)
  const skipOllama = routeToProvider._ollamaFailures > 1;
  ...
  } else if ((prov === 'ollama' || prov === PROVIDERS.OLLAMA) && !skipOllama) {
    return await callOllama(prompt, { system, model, temperature, maxTokens });
  }

  // Skip to next if Ollama failed too many times
  if (prov === 'ollama' || prov === PROVIDERS.OLLAMA) {
    routeToProvider._ollamaFailures = (routeToProvider._ollamaFailures || 0) + 1;
    throw new Error('Ollama skipped (multiple failures)');
  }

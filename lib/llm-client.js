...
const PROVIDER_TIMEOUT_MS = {
  [PROVIDERS.MINIMAX]: 60000,
  [PROVIDERS.GROQ]: 30000,
  [PROVIDERS.CLAUDE]: 90000,
  [PROVIDERS.OPENAI]: 60000,
  [PROVIDERS.OLLAMA]: 120000
};
...
async function callMiniMax(prompt, options = {}) {
  ...
  try {
    const response = await axios.post('https://api.minimax.io/v1/text/chatcompletion_v2', {
      ...
      timeout: PROVIDER_TIMEOUT_MS.MINIMAX
    });
...
async function callClaude(prompt, options = {}) {
  ...
  try {
    ...
    const response = await axios.post('https://api.anthropic.com/v1/messages', {
      ...
      timeout: PROVIDER_TIMEOUT_MS.CLAUDE
    });
...
async function callOpenAI(prompt, options = {}) {
  ...
  try {
    const response = await axios.post('https://api.openai.com/v1/chat/completions', {
      ...
      timeout: PROVIDER_TIMEOUT_MS.OPENAI
    });

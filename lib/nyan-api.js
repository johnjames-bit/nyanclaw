...
const NYAN_API_TOKEN = process.env.NYAN_API_TOKEN || 'CEzH8J3CpvL5B-Lvvb8iL1YSoBrxq1zM3YApXKcSknc';
 ...
 * @param {string} domain - Domain hint (psi-ema, legal, chemistry, etc.)
 * @returns {Promise<object>} API response
 */
async function atomicQuery(message, domain = null) {
  // Add domain hint if provided
  const fullMessage = domain 
    ? `${message} [${domain}]` 
    : message;

...
async function callNyanAPI(message, options = {}) {
  ...
  // Build payload with media if present
...
async function getPsiEMA(ticker) {
  ...
  const tickers = isArray ? ticker : [ticker];
  
  try {
    const response = await axios.post(
      `${NYAN_API_ENDPOINT}/psi-ema`,
      isArray ? { tickers } : { ticker: ticker },
      {
        headers: {
          'Authorization': `Bearer ${NYAN_API_TOKEN}`,
          'Content-Type': 'application/json'
    ...
    return {
      ...
      psiEma: result.psiEma || result.psi_ema_daily,
      psi_ema_daily: result.psi_ema_daily,
      psi_ema_weekly: result.psi_ema_weekly,

...
// Cloud metadata IP (169.254.169.254) - common across AWS/Azure/GCP
...
async function validateURLWithDNS(urlString) {
  ...
  try {
    ...
    // DNS resolution failure - block by default (prevents DNS rebinding)
    return { allowed: false, reason: `DNS resolution failed for ${hostname}: ${err.message}` };
  }
